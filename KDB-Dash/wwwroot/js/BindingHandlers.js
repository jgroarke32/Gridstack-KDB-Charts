            ...
            read: function () {
                ...
                for (var i = 0; i < widgets.length; i++) {
                    ...
                    if (item == null) {
                        ...
                        var widgetType = ko.utils.arrayFirst(vm.widgetType(), function (item) { 
                            return (item.TypeName === widgets[i].type);                   
                        });

                        grid.addWidget(itemElement, ko.unwrap(widgets[i].x), ko.unwrap(widgets[i].y), ko.unwrap(widgets[i].width), ko.unwrap(widgets[i].height), true, 4, widgetType.maxWidth, 5, widgetType.maxHeight, ko.unwrap(widgets[i].options().id));
